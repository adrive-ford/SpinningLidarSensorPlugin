...
class SPINNINGLIDARSENSORPLUGIN_API ASpinningLidarSensorActor : public AActor {
 ...
 public:
    ...
    // The filename that the lidar data will be written do.
    // The file will appear in the top level of your Unreal project folder.
    ...
    UPROPERTY(EditANywhere, Category = "Lidar Sensor Properties",
              meta = (UIMin = -90.f, UIMax = 180.f))
    float MaxElevation = 10.67f;

    // elevation angle in degrees for the lowest lidar beam
    UPROPERTY(EditANywhere, Category = "Lidar Sensor Properties",
              meta = (UIMin = -180.f, UIMax = 180.f))
    float MinElevation = -30.67f;

    /*Horizontal/Azimuth resolution in degrees.
     * For the Velodyne HDL-32E, assuming linear proportionality between angular resolution
     * and rotation rate, Rotation Rate in Hz = 50 * Angular Resolution in degrees.*/
    UPROPERTY(EditAnywhere, Category = "Lidar Sensor Properties",
              meta = (UIMin = 0.1f, UIMax = 0.4f))
    ...
    // which determines the probability that a given lidar hit will not be returned,
    // as a function of distance.
    UPROPERTY(EditAnywhere, Category = "Lidar Sensor Properties", meta = (UIMin = 0.f))
    float FalloffStdDev = 10.f;

    // If checked, lidar points in the output file will be in the
    // local coordinate frame of the sensor.
    ...
    // If checked, the color of the visualized lidar points will reflect the
    // intensity of the return, from red (low intensity) to green (high intensity).
    ...
    /*The number of "ticks" or frame updates per second if the sensor were to run in real time,
     * with one frame per angular position at which data is sampled.
     * For the Velodyne HDL-32E, 0.1 degree azimuth resolution at 5 Hz means 3600 frames/rotation,
     * so 3600*5=18000 FPS.0.4 degree resolution at 20 Hz is likewise 18000 FPS.
     * For slower than real time simulations, the recorded timestamps are scaled to account
     * for the ratio between real time and simulation time.*/
    UPROPERTY(EditAnywhere, Category = "Simulation Properties", meta = (UIMin = 1))
    float SimTimeFramerate = 18000.f;

    // UIMin ensures this value will never be <= 0.
    // ASSUMPTION: The frame rate required for real time will never be <1.

    // Choose a frame rate your machine can reliably achieve,
    // and the simulation will be capped at that.
    ...
    // The default pixel dimensions of the texture render target
    // used to render the base colors of the scene
    // from the perspective of the sensor, for use in calculating intensity values.
    // To change the dimensions of the render target,
    // open the render target itself and set these values directly.
    UPROPERTY()
 ...
 private:
    ...
    void GetLidarPointIntensity(FHitResult &Hit, TArray<FColor> &ImageBitmap,
                                TSharedPtr<FSceneView> SceneView, FColor &PointColorFromScene,
                                float &HitIntensity);
