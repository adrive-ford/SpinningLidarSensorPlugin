...
ASpinningLidarSensorActor::ASpinningLidarSensorActor()
{
	...
	//Root component: a dummy for other components to move relative to--for example, as the lidar scans, 
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> LidarMeshLoad(TEXT("/SpinningLidarSensorPlugin/Shape_Cylinder.Shape_Cylinder"));
	...
	static ConstructorHelpers::FObjectFinder<UMaterial> LidarMaterialLoad(TEXT("/SpinningLidarSensorPlugin/M_Basic_Wall.M_Basic_Wall"));
	...
	//Scale according to Velodyne dimensions, from the default cylinder mesh dimensions of 100cm height, 100cm diameter
    ...
    //Add a scene capture component to write the view from the sensor to a texture, so that colors can be sampled from the world at the beam locations.
    ...
    if (abs(MaxElevation - MinElevation) > 70.f) SceneCap->FOVAngle = abs(MaxElevation - MinElevation) + 20.f;
...
void ASpinningLidarSensorActor::BeginPlay()
{
    ...
    //Update the field of view for the scene capture if needed based on the max and min beam elevations
    float LidarFOV = abs(MaxElevation - MinElevation);
    if (LidarFOV > 70.f && LidarFOV < 150.f) { SceneCap->FOVAngle = abs(MaxElevation - MinElevation) + 20.f; }
    else if (LidarFOV >= 150.f) {
        SceneCap->FOVAngle = 170.f;
        UE_LOG(LogTemp, Warning, TEXT("Lidar beams have greater vertical FOV than the scene capture component! Intensities may not be calculated for points at the far top or bottom."));
    }
    ...
    if (FileHandle)
    {
        FString StringToWrite = FString(TEXT("timestamp (seconds),x (cm),y (cm),z (cm),intensity (scale of 0 to 255)") LINE_TERMINATOR);
    ...
    //Initialize the "sim time" value, which keeps track of the simulation clock regardless of whether the simulation runs in real time.
    SimTimeSeconds = 0.f;

    //Cap the frame rate at a value your machine can reliably achieve, to lock the simulation at a constant frame rate.
...
void ASpinningLidarSensorActor::Tick(float DeltaTime)
{
	...
	//Doing the max elevation beam outside the loop so that max elevation is as precise as possible, without rounding errors
	...
	//Increment the "sim time" value by one timestep, according to the frame rate of the sensor if it ran in real time
	SimTimeSeconds += 1.f / SimTimeFramerate;

	//Apply a relative rotation to the sensor. The mesh component will rotate while the root component is unchanged.
...
{
	//The time in seconds since the simulation began. By default, use "sim time" which may be slower than real time, 
	...
	FTextureRenderTargetResource* RenderTextureResource = SceneCap->TextureTarget->GameThread_GetRenderTargetResource();
...
//            GetLidarPointIntensity(Hit, ImageBitmap, SceneView, PointColorFromScene, HitIntensity);
            ...
            //If the user has chosen to use the sensor's local coordinates, transform into this frame.
...
/*Set up a FSceneView to match the perspective of the scene capture component, so that its WorldToPixel function can be used
to find the pixel location of each lidar point*/
void ASpinningLidarSensorActor::GetSceneView(USceneCaptureComponent2D * SceneCapture, TSharedPtr<FSceneView> &OutSceneView)
{
    ...
    FSceneViewFamilyContext ViewFamilyContext(FSceneViewFamily::ConstructionValues(
        SceneCapture->TextureTarget->GameThread_GetRenderTargetResource(),
    ...
    ViewTransform.SetTranslation(FVector::ZeroVector); //remove translation to get the transform for the rotation only
    SceneViewOptions.ViewRotationMatrix = ViewTransform.ToInverseMatrixWithScale() *
        //Switch between screen coordinates (y-up, x-right, z-forward) and Unreal's left-handed 3D coordinates (z-up, y-right, x-forward)
    ...
    float HalfFOV = SceneCapture->FOVAngle * (float)PI / 360.f; //Half the field of view in radians
    float MultFOVX;
    float MultFOVY;
    ...
    SceneViewOptions.ProjectionMatrix = FPerspectiveMatrix(HalfFOV, HalfFOV, MultFOVX, MultFOVY, GNearClippingPlane, GNearClippingPlane);
...
void ASpinningLidarSensorActor::AddGaussianRangeNoise(FHitResult &Hit)
{
	if (Hit.bBlockingHit) {
		...
		FVector BeamUnitVector = UKismetMathLibrary::GetDirectionUnitVector(GetActorLocation() + GetActorUpVector()*BeamStartRelativeZ, Hit.ImpactPoint);

		//Standard deviation.  
		//ASSUMPTION: the range noise is greatest when the angle of incidence is closest to parallel with the surface.
		//This function should be adjusted to match real data, but it has a standard deviation based on the range accuracy from the spec sheet 
		//which is minimized when the beam is perpendicular to the surface and maximized as the beam approaches parallel to the surface.
		float StdDev = RangeAccuracy * (1.f - FVector::DotProduct(-BeamUnitVector, Hit.ImpactNormal));
...
//Simulate the probability that there will be no return signal received for some hits, especially near max range.
...
void ASpinningLidarSensorActor::RandomizeWhetherHitReturns(FHitResult &Hit)
{
	if (Hit.bBlockingHit && FalloffStdDev > 0.f) {
		...
		float ProbabilityOfNoReturn = ProbabilityOfNoReturn = MaxRangeNoReturnProbability * exp(-0.5f * pow((Hit.Distance - LidarRange) / FalloffStdDev, 2));
...
{
	//an out parameter of LineTraceSingleByChannel that will contain the data returned from a firing of a laser
	FHitResult Hit;

	//The raycast starts at a location that is an adjustable distance along the actor's z axis from the actor's root component.
	...
	FVector BeamEnd = BeamStart + 
		LidarMeshComponent->GetForwardVector().RotateAngleAxis(BeamElevation, -LidarMeshComponent->GetRightVector())*LidarRange;
	//Note: Unreal uses a left-handed coordinate system, so the "right" vector is multiplied by -1 before rotating about it

	//Raycasting parameters: "true" to trace using full visible geometry, "this" so that the sensor itself does not occlude the beam
...
//Input a lidar hit and a bitmap of the render texture showing the lidar's view. The color and intensity of the hit are out parameters.
void ASpinningLidarSensorActor::GetLidarPointIntensity(FHitResult &Hit, TArray<FColor> &ImageBitmap, TSharedPtr<FSceneView> SceneView, FColor &OutPointColorFromScene,  float &OutHitIntensity)
{
	OutHitIntensity = 0.f;
	if (Hit.bBlockingHit) {
		...
		//NOTE: if no base color can be found for a lidar point, the intensity will remain at zero by default.
		//This issue should only happen if the difference between max and min beam elevation is greater than 
		//the field of view for the scene capture component.
		...
		//The magnitude of the effect of angle of incidence is adjustable in the editor, for efficiency of refining the model to match real data.
		//multiply by a factor based on angle of incidence, so that returns are brightest when the beam is perpendicular to the surface.
		FVector BeamUnitVector = UKismetMathLibrary::GetDirectionUnitVector(GetActorLocation() + GetActorUpVector()*BeamStartRelativeZ, Hit.ImpactPoint);
		OutHitIntensity *= IntensityAffectedByAngle * FVector::DotProduct(-BeamUnitVector, Hit.ImpactNormal) + (1.f- IntensityAffectedByAngle);

		//Set the lidar point color for visualization on a scale from red to green where green is most intense and red is least.
		if (bVisualizeIntensity) OutPointColorFromScene = FColor::MakeRedToGreenColorFromScalar(OutHitIntensity / 255.f);
	}
}
...
{
	//For visualization, determine whether to draw the beam ending at the impact point or at max range
